...
OPTIONS:
  ...
  -s, --scope       Specify scope for commit message
EXAMPLES:
  gen-commit.sh                    # Staged changes only (default)
  gen-commit.sh -a                 # All changes (staged + unstaged)
  gen-commit.sh -c                 # Auto-commit staged
  gen-commit.sh -p                 # Print only
  gen-commit.sh -s frontend        # Use 'frontend' scope
...
generate_commit_message() {
  ...
  if [[ -n "$SCOPE_NAME" ]]; then
    scope_instruction="- Use scope ($SCOPE_NAME)"
  else
    scope_instruction="- Do not include a scope, unless obvious from previous commits and current changes."
  fi

  local last_3_git_messages
  last_3_git_messages=$(git log --pretty=format:'%s' -n 3 2>/dev/null)
...
- scope is optional, but is always between parentheses
$scope_instruction
...
# ---------------------------------------------------------------------------
choose_scope() {
  # If scope is already provided, use it
  [[ -n "$SCOPE_NAME" ]] && { echo "$SCOPE_NAME"; return; }
...
# Option parsing
# ---------------------------------------------------------------------------
OPTS=$(getopt -o hm:pDacvs: --long help,model:,print,debug,all,commit,verbose,scope: -n "$0" -- "$@")
eval set -- "$OPTS"
...
main() {
  ...
  INTERACTIVE=false
  if [[ "$AUTO_COMMIT" != true || -z "$SCOPE_NAME" ]]; then
    INTERACTIVE=true
  fi
  ...
  print_debug "Flags: MODEL='$MODEL', ALL_CHANGES=$ALL_CHANGES, AUTO_COMMIT=$AUTO_COMMIT, SCOPE_NAME='$SCOPE_NAME', PRINT_ONLY=$PRINT_ONLY, INTERACTIVE=$INTERACTIVE"

  check_requirements
  check_git_repo

  # Determine mode (staged by default, all if -a/--all or interactive prompt)
  local mode="staged"
  ...
  local scope
  scope=$(choose_scope "$SCOPE_NAME")
  print_debug "Scope: '$SCOPE_NAME'"

  # Generate message
  local message
  message=$(generate_commit_message "$changes" "$SCOPE_NAME")
